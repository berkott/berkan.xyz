...
Looking at the "Guided" (I'm using quotes because it is technically not guided backprop just the gradient of the input, explained more above) Grad-CAM output, it is clear how that can be mistaken as a 2. The heatmap also shows that the model isn't paying attention to the bottom of the digit to classify it as a 2. 
...
The CAM model didn't look at the base of the 4 as much, but the Grad-CAM model clearly does, which could be an explanation for why it got this classification correct.
